...
// Ordinal color scale (used for categorical variables in D3) for player position color mapping
var customColors = ['#ff5733', '#ffbd69', '#45aaf2', '#2ecc71'];
...
var toolTip = d3.tip()
    ...
    .html(function (event, d) {
        ...
        return "<h5>" + d['name'] + "</h5><table><thead><tr><td>Position</td><td>Age</td><td>Goals Scored</td></tr></thead>"
            + "<tbody><tr><td>" + d['position'] + "</td><td>" + d['age'] + "</td><td>" + d['goals scored'] + "</td></tr></tbody>"
            + "<thead><tr><td>Hours Played</td><td>Assists</td><td>Goals Conceded</td><td>Yellow Cards</td></tr></thead>"
            + "<tbody><tr><td>" + d['time played (hours)'] + "</td><td>" + d['assists'] + "</td><td>" + d['goals conceded'] + "</td><td>" + d['yellow cards'] + "</td></tr></tbody></table>"
    });
...
SplomCell.prototype.update = function (g, data) {
    ...
    var dots = cell.selectAll('.dot')
        .data(data, function (d) {
            return d.name + '-' + d.age + '-' + d.assists; // Create a unique id for the player
        });
...
d3.csv('epl_players.csv', dataPreprocessor).then(function (dataset) {
